...
| `agroapp.yml` | lint (**advisory**, 7 pre-existing errors), `pnpm build`, then `tsc --noEmit`, Docker build |
| `publish-images.yml` | Pushes both images to GHCR on `master` and version tags |

In `agroapp.yml`, `tsc` runs **after** `pnpm build`, and the order is required.
`next-env.d.ts` declares the `*.svg`/`*.png` module types that eight files
import; it is generated by the build and gitignored, so on a clean checkout
running `tsc` first fails with eight `TS2307`s that reproduce nowhere locally.

`publish-images.yml` **skips** the `agroapp` image when
`NEXT_PUBLIC_SUPABASE_URL` or `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` are unset
as repository variables, rather than failing — a fork has no variables and should
not report a misconfiguration it does not have. Placeholders are deliberately not
substituted: an image with placeholder Supabase config looks deployable and fails
